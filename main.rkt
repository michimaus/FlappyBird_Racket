...
(define-struct birdStr (y fallY) #:transparent)
(define-struct pipeStr (x gapY) #:transparent)
(define-struct variablesStr (grv mom scroll) #:transparent)
(define-struct abilitiesStr (visible active) #:transparent)

(define myInitialBird (birdStr bird-initial-y 0.0))
(define myInitialPipe (pipeStr scene-width (+ added-number (random random-threshold))))
(define myInitialVariables (variablesStr initial-gravity initial-momentum initial-scroll-speed))


;(define myInitialAbilities (abilities ) )
...
(define-struct stateStr (birdStr pipeStr variablesStr score))
...
(define (get-initial-state)
  (stateStr (birdStr bird-initial-y 0.0)
            (list (pipeStr scene-width (+ added-number (random random-threshold))))
            (variablesStr initial-gravity initial-momentum initial-scroll-speed)
            0))
...
(define (next-state-bird bird gravity)
  (struct-copy birdStr bird
               [y (+ (birdStr-y bird) (birdStr-fallY bird))]
               [fallY (+ gravity (birdStr-fallY bird))]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy birdStr bird [fallY (* -1 momentum)]))
...
(define (change current-state pressed-key)
  (match-let (((stateStr bird pipes variables _) current-state))
    (if (key=? pressed-key " ")
        (struct-copy stateStr current-state (birdStr
                                             (next-state-bird-onspace bird (variablesStr-mom variables))))
        current-state)
    ))
...
(define (move-pipes pipes scroll-speed)
  (map (λ (pipe) (struct-copy pipeStr pipe (x (- (pipeStr-x pipe) scroll-speed)))) pipes))
...
(define (clean-pipes pipes)
  (filter (λ (pipe) (< 0.0 (+ pipe-width (get-pipe-x pipe)))) pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (append pipes (list (pipeStr (+ (+ (pipeStr-x (last pipes)) pipe-width) pipe-gap)
...
(define (next-state-pipes pipes scroll-speed)
  (let* ((a (move-pipes pipes scroll-speed))
         (b (clean-pipes a))
         (c (add-more-pipes b)))
    c))
...
(define (check-ground-collision bird)
 (if (>= (+ (birdStr-y bird) bird-height) ground-y) #t #f))
...
(define (invalid-state? state)
  (let* ((bird (stateStr-birdStr state))
         (pipes (stateStr-pipeStr state)))
    (if (or (check-ground-collision bird) (check-pipe-collisions bird pipes))
        #t #f)))
...
(define (check-pipe-collisions bird pipes)
  (ormap (λ (pipe)
           (let* ([birdXlt bird-x]
                  [birdXrh (+ bird-x bird-width)]
                  [birdYtop (birdStr-y bird)]
                  [birdYbot (+ birdYtop bird-height)]
                  [pipeXlt (pipeStr-x pipe)]
                  [pipeXrh (+ pipeXlt pipe-width)]
                  [pipeYtop (pipeStr-gapY pipe)]
                  [pipeYbot (+ pipeYtop pipe-self-gap)]
                  [cornerYtop 0]
                  [cornerYbot ground-y])
             (or
              (check-collision-rectangles (make-posn birdXlt birdYtop)
                                          (make-posn birdXrh birdYbot)
                                          (make-posn pipeXlt pipeYbot)
                                          (make-posn pipeXrh cornerYbot))
              (check-collision-rectangles (make-posn birdXlt birdYtop)
                                          (make-posn birdXrh birdYbot)
                                          (make-posn pipeXlt cornerYtop)
                                          (make-posn pipeXrh pipeYtop)))))
         pipes))
...
(define (next-state state)
  (match-let ([(stateStr bird pipes variables thescore) state])
    (struct-copy stateStr state
                 [birdStr (next-state-bird bird (variablesStr-grv variables))]
                 [score (+ thescore 0.1)]
                 [pipeStr (next-state-pipes pipes (variablesStr-scroll variables))])))
...
(define (draw-frame state)
  (match-let ([(stateStr bird pipes variables thescore) state])
    (let* ((a (place-pipes pipes initial-scene))
           (b (place-image (score-to-image thescore) text-x text-y a))
           (c (place-image ground-image
                           (quotient scene-width 2)
                           (+ ground-y (quotient ground-height 2)) b))
           (d (place-image bird-image
                           (+ bird-x (quotient bird-width 2))
                           (+ (birdStr-y bird) (quotient bird-height 2)) c)))
      d))
  )
...
(define (place-pipes pipes scene)
  (foldr (λ (pipe res) (match-let (((pipeStr xIn yIn) pipe))
                     (let* ((x (+ xIn (quotient pipe-width 2)))
                           (y (- yIn (quotient yIn 2))))
                       (place-image (rectangle pipe-width yIn "solid" "green") x y res))))
         (foldr (λ (pipe res) (match-let (((pipeStr xIn yIn) pipe))
                     (let* ((x (+ xIn (quotient pipe-width 2)))
                           (yBot (+ yIn pipe-self-gap))
                           (y ( + yBot (quotient (- scene-height yBot) 2))))
                       (place-image (rectangle pipe-width (- scene-height yBot) "solid" "green") x y res)))) scene pipes)
         pipes))
